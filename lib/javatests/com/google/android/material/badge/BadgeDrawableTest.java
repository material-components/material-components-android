...
public class BadgeDrawableTest {
  ...
  private static final float TEST_BADGE_HORIZONTAL_OFFSET = 10;
  private static final float TEST_BADGE_VERTICAL_OFFSET = 5;
  ...
  public void testSavedState() {
    ...
    badgeDrawable.setBadgeHorizontalOffset(TEST_BADGE_HORIZONTAL_OFFSET);
    badgeDrawable.setBadgeVerticalOffset(TEST_BADGE_VERTICAL_OFFSET);
    ...
    // badge offsets
    assertThat(restoredBadgeDrawable.getBadgeHorizontalOffset())
        .isEqualTo(TEST_BADGE_HORIZONTAL_OFFSET);
    assertThat(restoredBadgeDrawable.getBadgeVerticalOffset())
        .isEqualTo(TEST_BADGE_VERTICAL_OFFSET);
  }
  ...
  @Test
  public void testBadgeHorizontalOffset() {
    testBadgeOffsetValueHelper(
        R.xml.standalone_badge_offset,
        R.styleable.Badge_badgeHorizontalOffset,
        dp2px(TEST_BADGE_HORIZONTAL_OFFSET)
    );
  }

  @Test
  public void testBadgeVerticalOffset() {
    testBadgeOffsetValueHelper(
        R.xml.standalone_badge_offset,
        R.styleable.Badge_badgeVerticalOffset,
        dp2px(TEST_BADGE_VERTICAL_OFFSET)
    );
  }
  ...
  private void testBadgeOffsetValueHelper(
      @XmlRes int xmlId,
      @StyleableRes int offsetIndex,
      float expectedValue
  ) {
    TypedArray a = getTypedArray(xmlId);

    float value = 0;
    if (a.hasValue(offsetIndex)) {
      value = a.getDimension(offsetIndex, 0f);
    }
    assertThat(value).isEqualTo(expectedValue);
    a.recycle();
  }
