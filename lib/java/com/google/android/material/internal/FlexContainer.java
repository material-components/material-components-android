...
/**
 * An interface that has the common behavior as the flex container}.
 *
 * @hide
 */
...
interface FlexContainer {
  ...
  /**
   * Returns a flex item as a View, which is reordered by taking the order attribute into account.
   *
   * @param index the index of the view
   * @return the reordered view, which order attribute is taken into account. If the index is
   *     negative or out of bounds of the number of contained views, returns {@code null}.
  ...
  /**
   * @return the list of the flex lines including dummy flex lines (flex line that doesn't have any
   *     flex items in it but used for the alignment along the cross axis), which aren't included in
   *     the {@link FlexContainer#getFlexLines()}.
   */
